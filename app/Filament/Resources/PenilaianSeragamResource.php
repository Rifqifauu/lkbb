...
class PenilaianSeragamResource extends Resource
{
    ...
    ->label('Pilih Peserta')
    ->options(function () {
        return Peserta::whereDoesntHave('penilaianSeragam', function ($query) {
        ...
        ->schema([
            Radio::make('nilai')
                ...
                ->options(function (callable $get) {
                    ...
                    $aspek = AspekSeragam::find($aspekId);
                    if (!$aspek) return [];
    ...
    public static function table(Table $table): Table
{
    ...
    foreach (AspekSeragam::all() as $aspek) {
        $columns[] = Tables\Columns\TextColumn::make("aspek_{$aspek->id}")
            ->label($aspek->nama_penilaian)
            ->getStateUsing(function ($record) use ($aspek) {
                // ambil dari relasi peserta -> penilaianSeragam (sudah di-load)
                $penilaian = $record->peserta->penilaianSeragam
    ...
    return $table
        ->query(
            PenilaianSeragam::with([
                'peserta.penilaianSeragam', // load semua penilaian peserta
                'penilai',
                'aspek'
            ])

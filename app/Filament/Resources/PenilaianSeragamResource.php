...
use App\Filament\Resources\PenilaianSeragamResource\RelationManagers;
...
public static function table(Table $table): Table
{
    ...
    foreach (AspekSeragam::all() as $aspek) {
        $columns[] = Tables\Columns\TextColumn::make("aspek_{$aspek->id}")
            ...
            ->getStateUsing(function ($record) use ($aspek) {
                ...
                $penilaian = PenilaianSeragam::where('id_peserta', $record->id_peserta)
    ...
    return $table
        ->query(
            ...
            PenilaianSeragam::with(['peserta', 'penilai', 'aspek'])
                ->orderBy('id_peserta')
                ->orderBy('id_user')
        )

...
class CreatePenilaianVariasiFormasi extends CreateRecord
{
...
protected function handleRecordCreation(array $data): Model
{
    ...
    foreach ($penilaianItems as $item) {
        if (!empty($item['nilai']) && !empty($item['id_aspek'])) {
            // Ambil Aspek
            $aspek = AspekVariasiFormasi::find($item['id_aspek']);
            ...
            // Cek record lama
            $existingRecord = PenilaianVariasiFormasi::where('id_peserta', $pesertaId)
    ...
    ->first();
            ...
            if ($existingRecord) {
                ...
                $record = PenilaianVariasiFormasi::create([
    ...
    return $createdRecords[0] ?? new PenilaianVariasiFormasi();
}
